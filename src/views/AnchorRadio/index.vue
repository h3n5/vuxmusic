  ...
  <div class="mContent">
    ...
    <div class="songList">
      <button-tab class="tag">
        <button-tab-item @on-item-click="tagChange">
          <x-icon type="ios-keypad" size="14" class="icon" fill="#999"></x-icon>电台分类
        </button-tab-item>
        <button-tab-item @on-item-click="tagChange">
          <x-icon type="ios-list" size="14" class="icon" fill="#999"></x-icon>电台排行
        </button-tab-item>
      </button-tab>
      <div class="listItem">
        <div class="title">
          ...
          <p class="random">
            <x-icon type="ios-refresh" fill="#999"></x-icon>
          </p>
        </div>
        <radioList v-for="(item, index) in songs" :key="index" class="content" :song="item"></radioList>
      </div>
    </div>
  </div>
</template>
<script>
import { getBanner, getpersonalized, getSongListByOrder } from '@/api/api'
